...
你可能听过那个关于金鱼的笑话——据说金鱼的记忆只有七秒，每绕鱼缸一圈，眼前的世界就是全新的。

虽然这个说法并不科学（金鱼的记忆实际上可以持续好几个月），但它揭示了一个有趣的思维实验：**如果每次面对问题时都能以全新的视角审视，会发生什么？**

在 Ralph 循环技术中，这不是笑话，而是一个核心设计原则——**Fresh Context（新鲜上下文）**。

## 什么是"新鲜上下文"？

### 每次迭代都从零开始

想象你是一个侦探，正在调查一起复杂的案件。

**传统方式**就像你一直盯着同一块白板，上面贴满了线索、照片、红线和便利贴。随着时间推移，白板越来越乱，某些早期的错误假设可能已经深深嵌入你的思维中，但你已经看不出它们有什么问题了——因为你"习惯了"。

**新鲜上下文方式**则完全不同。每天早上，你走进办公室，面对的是一块**干净的白板**。所有的证据都整齐地放在档案柜里（磁盘上的文件），你需要重新阅读它们，重新分析，重新建立联系。

听起来效率低下？

恰恰相反，这种"失忆"反而成为了一种超能力。
...
假设你正在调试一个复杂的 bug。第一次尝试时，你怀疑是数据库连接的问题，花了两小时证明这条路走不通。第二次，你认为是缓存导致的，又花了一小时排除。第三次……

此时，你的脑子里已经塞满了各种"这个不行"、"那个试过了"的记忆。这些记忆不仅占用你的"内存"，还可能形成**思维定势**——你开始避开某些可能的方向，仅仅因为它们"看起来像"之前失败的路径。
...
那些曾经的"死路"？对不起，它根本不记得。这意味着如果第一次判断错误地排除了正确答案，第二次迭代完全可能重新发现它。
...
AI 在每次迭代开始时都会重新阅读提示词（PROMPT.md）、检查代码状态、分析测试结果。这个过程就像每次考试都重新读一遍题目——你可能会注意到之前忽略的关键细节。
...
为什么这么简单的答案会被忽略？因为我们的大脑习惯性地把"装图钉的盒子"视为"装东西的容器"，而不是"可以利用的工具"。
...
每次迭代开始时，AI 会重新读取这些文件。就像那个侦探每天早上面对干净的白板，但档案柜里保存着所有的证据——他会重新阅读它们，而不是依赖自己可能有偏差的记忆。
...
每次迭代都像面对一个空房间——你可以完全重新规划，不受过去错误决定的束缚。
...
在迭代 2 中，因为没有"A 不是问题"这个包袱，AI 能够以全新的眼光审视代码，发现真正的问题所在。
...
这种设计看似反直觉——我们不是一直被教导要"吃一堑长一智"吗？但在 AI 自主工作的场景中，新鲜上下文证明了：有时候，战略性的"失忆"反而是通往成功的捷径。
